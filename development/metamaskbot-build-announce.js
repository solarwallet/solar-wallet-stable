...
async function start () {
  ...
  const CHROME = `${BUILD_LINK_BASE}/builds/solarwallet-chrome-${VERSION}.zip`
  const FIREFOX = `${BUILD_LINK_BASE}/builds/solarwallet-firefox-${VERSION}.zip`
  const EDGE = `${BUILD_LINK_BASE}/builds/solarwallet-edge-${VERSION}.zip`
  const OPERA = `${BUILD_LINK_BASE}/builds/solarwallet-opera-${VERSION}.zip`
  ...
  const POST_COMMENT_URI = `https://api.github.com/repos/poanetwork/solar-wallet/issues/${CIRCLE_PR_NUMBER}/comments`
